# admin_simple.py - Simplified admin area without auth dependencies
...
async def admin_dashboard_page(request: Request, auth: bool = Depends(check_admin_auth)):
...
async def add_pattern(
    ...
    """Add a new pattern to custom patterns"""
    try:
        custom_patterns = load_custom_patterns()
        ...
        print(f"Error in add_pattern: {e}")
        raise HTTPException(status_code=500, detail=f"Internal server error: {str(e)}")
